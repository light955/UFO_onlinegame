...
socket.on('state', (players, bullets, walls) => {
    ...
    Object.values(players).forEach((player) => {
        ...
        playerMesh.position.set(40,40,40);
        //playerMesh.position.set(90,40,90);
        ...
        if (player.socketId === socket.id) {
            ...
            //xが0yが0のときは-110,200,40
            //初期値は-150,200,0
            //45度ときは-66.06601717798213 200 -66.06601717798212
          
            camera.position.set(
                player.x + player.width/2 - 90 * Math.cos(player.angle),
                200,
                player.y + player.height/2 - 90 * Math.sin(player.angle)
            );
